                ...
                <div class="card-header bg-card-header text-light text-uppercase mb-3"><h5><i class="fas fa-plus"></i> Adicionar categoria</h5></div>
                    <div class="card-body">
                        ...
                        <div class="alert alert-danger my-3" role="alert"><strong><i class="fas fa-times"></i> Mensagem de erro!</strong> Suas alterações não foram concluídas!</div>
                            @if(isset($c))
                                <form class="form-group form-group-alt" action="{{route('categorias.update',$c->id)}}" method="POST">
                                <input type="hidden" name="_method" value="PUT" />
                            @else
                                <form class="form-group form-group-alt" action="{{route('categorias.store')}}" method="POST">
                            @endif
                                @csrf
                                <div class="row px-md-5">
                                    ...
                                    <div class="col-md-12 mt-3">												
                                        <input type="submit" class="btn btn-primary d-block m-auto text-uppercase" data-toggle="modal" data-target="#modal" value="Salvar Dados"/></br>											
                                    </div>
                                </div>
                            </form>	
                        </div>								
